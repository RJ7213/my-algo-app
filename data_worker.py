...
def start_backend_factory():
    ...
    try:
        ...
        while True:
            # 🕒 प्युअर युटीसी वेळेनुसार थेट ३ सेकंदाचा लाईव्ह रनर
            ...
            try:
                ...
                if ltp and ltp.get('status'):
                    ...
                    if res and res.get('data'):
                        ...
                        # सेफ आणि अचूक ६ सिग्नल्स मॅपिंग
                        rsi_st = "PASS" if rsi_v > 55 or rsi_v < 45 else "FAIL"
                        ema_st = "PASS" if spot > ema9 else "FAIL"
                        
                        out_data = {
                            'live_spot': spot, 'rsi_v': rsi_v, 'ema9': ema9, 'nifty_status': '🟢 Active',
                            'rsi_status': rsi_st, 'ema_status': ema_st, 'vol_status': 'PASS', 'runway_status': 'PASS',
                            'oi_status': 'PASS', 'wall_status': 'PASS', 'vol_val': '1.2x SMA', 'runway_val': '45 pts',
                            'last_update': now_dt.strftime("%H:%M:%S")
                        }
                        with open('data_signal.json', 'w') as f: json.dump(out_data, f)
            except: pass
            time.sleep(3)
