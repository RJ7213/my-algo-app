...
def start_indicator_engine():
    while True:
        ...
        try:
            ...
            if is_rejection: otype, rsi_st, setup_name = ("PE" if top_wick > bot_wick else "CE"), "PASS", "Major Rejection"
            elif is_pullback: otype, rsi_st, setup_name = ("PE" if spot < ema9 else "CE"), ("PASS" if 30<=rsi_v<=55 else "FAIL"), "Pullback"
            else: otype, rsi_st, setup_name = ("CE" if rsi_v >= 60 else ("PE" if rsi_v <= 40 else "NONE")), ("PASS" if rsi_v>=60 or rsi_v<=40 else "FAIL"), "Breakout"
            
            ttype = f"{otype}_BUY" if otype != "NONE" else "NONE"
            ema_dist = abs(spot - ema9)
            ema_st = "PASS" if (otype=="CE" and spot>=(ema9-3) and ema_dist<=22) or (otype=="PE" and spot<=(ema9+3) and ema_dist<=22) else "FAIL"
            ...
            reason = f"Automated {setup_name} Monitor (Runway: {run_df:.1f} pts)"
            
            with open('strategy_signal.json', 'w') as f:
                json.dump({'live_spot': spot, 'rsi_v': rsi_v, 'ema9': ema9, 'rsi_status': rsi_st, 'ema_status': ema_st, 'vol_status': vol_st, 'runway_status': runway_st, 'vol_val': f"{vol_ratio}x SMA", 'runway_val': f"{run_df:.1f} pts", 'intraday_high': high, 'intraday_low': low, 'algo_reason': reason, 'signal_triggered': all_p, 'trade_type': ttype, 'otype': otype, 'next_w': next_w, 'run_df': run_df, 'c_low': c_low, 'c_high': c_high}, f)
        except: pass
        time.sleep(2)
