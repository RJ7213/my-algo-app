...
def start_backend_factory():
    ...
    # websocket-client calls on_close with status code and message, while
    # some SmartAPI releases accept only wsapp. Normalize that SDK callback.
    def smartws_close_compat(
        self,
        wsapp,
    ...
    SmartWebSocketV2._on_close = smartws_close_compat
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    # Restart the full worker with a fresh API session if the
                    # SDK thread exits after exhausting its internal retries.
                    if not ws_thread.is_alive():
                        raise RuntimeError(
                            "SmartWebSocketV2 thread stopped"
                        )
