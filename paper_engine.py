    ...
    while True:
        ...
        try:
            ...
            if act:
                ...
                # 🎯 नवीन ट्रेड एक्झिक्युशन जेव्हा इंजिन २ए सिग्नल देईल [Claim]
                if strat['signal_triggered']:
                    strike = int(round(spot / 50) * 50)
                    o_sym = f"NIFTY {strike} {strat['otype']}"
                    p_entry = 100.0
                    p_sl, p_targ = p_entry - 15.0, p_entry + max(30.0, strat['run_df'] * 0.50)
                    lots = max(1, int(2000 / 15.0 / 75))
                    
                    manage_ledger(new_t={'time': now_dt.strftime("%H:%M:%S"), 'type': strat['trade_type'], 'option_symbol': o_sym, 'option_token': "142500", 'entry': p_entry, 'sl': p_sl, 'target': p_targ, 'index_entry': spot, 'index_sl': spot-20 if strat['otype']=="CE" else spot+20, 'index_target': strat['next_w'], 'qty': lots*75, 'status': 'ACTIVE', 'exit_price': 0.0, 'pnl_realized': 0.0})
            
            # डॅशबोर्डसाठी फायनल सिग्नल्स एकत्र करणे
            ...
            with open('paper_signal.json', 'w') as f:
                json.dump({'rsi_status': strat['rsi_status'], 'ema_status': strat['ema_status'], 'vol_status': strat['vol_status'], 'runway_status': strat['runway_status'], 'vol_val': strat['vol_val'], 'runway_val': strat['runway_val'], 'intraday_high': strat['intraday_high'], 'intraday_low': strat['intraday_low'], 'algo_reason': f"🏃 Jackpot Active: {current_act['option_symbol']}" if current_act else strat['algo_reason'], 'signal_active': current_act is not None, 'active_trade_symbol': current_act['option_symbol'] if current_act else 'NONE'}, f)
        except: pass
        time.sleep(2)
