...
def load_live_signal():
    if not os.path.exists('data_signal.json'): 
        return {'live_spot': 24128.75, 'rsi_v': 40.4, 'ema9': 24147.88, 'nifty_status': '⏳ Waiting...', 'rsi_status': 'FAIL', 'ema_status': 'FAIL', 'vol_status': 'FAIL', 'runway_status': 'FAIL', 'oi_status': 'FAIL', 'wall_status': 'FAIL', 'vol_val': '1.2x SMA', 'runway_val': '45 pts', 'oi_val': '1.0x', 'depth_val': '0%', 'intraday_high': 24297.45, 'intraday_low': 24128.75, 'algo_reason': 'Starting...', 'signal_active': False, 'trade_type': 'NONE', 'entry_p': 0.0, 'sl_p': 0.0, 'target_p': 0.0, 'option_symbol': 'NONE', 'lots_suggested': '0', 'risk_cash': '₹2000', 'last_update': '00:00:00'}
    try:
...
# 🎫 लाइव्ह जॅकपॉट कॉल कार्ड पॉप-अप लॉजिक (Direct Option Premium Display) [Claim]
trade_card_html = ""
if data.get('signal_active', False):
    card_color = "#00e676" if "CE" in data.get('option_symbol','') else "#ff5252"
    trade_card_html = f"""
    <div style="background:linear-gradient(135deg, {card_color}15, #060814); border:2px solid {card_color}; padding:12px; border-radius:12px; color:white; font-family:-apple-system,BlinkMacSystemFont,sans-serif; margin-top:12px; line-height:1.4;">
        <div style="display:flex; justify-content:space-between; font-size:11px; color:{card_color}; font-weight:bold; margin-bottom:6px;">
            <span>🚨 ACTIVE POSITION: {data.get('option_symbol')}</span>
            <span style="background:{card_color}20; padding:2px 6px; border-radius:4px;">Live Ride</span>
        </div>
        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:8px; text-align:center; margin-bottom:8px;">
            <div style="background:#111422; padding:6px; border-radius:6px; border:1px solid #1c2136;">
                <span style="font-size:9px; color:#8f96a3; display:block;">PREMIUM ENTRY</span>
                <b style="font-size:14px; color:#00e676;">₹{data.get('entry_p', 0.0):.2f}</b>
            </div>
            <div style="background:#111422; padding:6px; border-radius:6px; border:1px solid #1c2136;">
                <span style="font-size:9px; color:#8f96a3; display:block;">PREMIUM SL</span>
                <b style="font-size:14px; color:#ff5252;">₹{data.get('sl_p', 0.0):.2f}</b>
            </div>
            <div style="background:#111422; padding:6px; border-radius:6px; border:1px solid #1c2136;">
                <span style="font-size:9px; color:#8f96a3; display:block;">JACKPOT TARGET</span>
        ...
        <div style="background:#111422; padding:6px; border-radius:6px; font-size:10px; border:1px dashed {card_color}50; text-align:center;">
            🧮 <b>Auto Size:</b> <span style="color:#00e676; font-weight:bold;">{data.get('lots_suggested', '0 Lots')}</span> | Max Capital Risk Allowed: {data.get('risk_cash', '₹2000')}
        </div>
    </div>"""

dhan_html = f"""
<div style="background-color:#060814; padding:15px; border-radius:12px; color:white; border: 1px solid #1c2136; font-family:-apple-system,BlinkMacSystemFont,sans-serif; line-height: 1.4; height: 530px;">
    ...
    <div style="background-color:#00e67610; border:1px solid #00e67650; padding:10px; border-radius:10px; text-align:center; margin-bottom:12px;">
        <h1 style="font-size:34px; margin:2px 0; color:#00e676; font-weight:bold;">{data.get('live_spot'):.2f}</h1>
    </div>
    <table style="width:100%; font-size:12px; border-collapse:collapse; background:#111422; border-radius:10px; overflow:hidden; border: 1px solid #1c2136;">
        ...
        <tbody>
            ...
            <tr><td style="padding:6px 8px;">2. Institutional 9 EMA</td><td style="padding:6px 8px; text-align:center; color:#fff;">{data.get('ema9', 0.0):.2f}</td><td style="padding:6px 8px; text-align:right;">{map_pf(data.get('ema_status'))}</td></tr>
    ...
    <div style="background:#1c2136; border-radius:8px; padding:8px; font-size:11px; margin-top:10px; border-left:4px solid #ffb300; color:#e2e5ec;">🧠 <b>ALGO LIVE ANALYZER:</b> {data.get('algo_reason')}</div>
...
components.html(dhan_html, height=550, scrolling=False)
