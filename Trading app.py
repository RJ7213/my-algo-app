...
if st.session_state['master_unlocked']:
    ...
    if st.session_state['is_connected'] and st.session_state['smartApi']:
        ...
        if current_time >= m_close or current_time < m_settle:
            session_status, sig_color, f_days, t_hour = "🔒 MARKET CLOSED (DISPLAYING LAST SESSION TRADES)", "#ff5252", 3, "15:30"
        elif m_settle <= current_time < m_open:
            session_status, sig_color, f_days, t_hour = "🗓️ PRE-MARKET SETTLEMENT ACTIVE", "#ffb300", 1, "09:08"
        else:
            session_status, sig_color, f_days, t_hour = "⏳ ALGO SCALPING SCANNERS ACTIVE... WAITING FOR 15-PT BREAKOUT", "#8f96a3", 2, "%H:%M"
        ...
        if is_nifty_ok and is_crude_ok:
            ...
            session_status = "⏳ ALGO SCALPING RUNNING... (CONNECTIVITY RE-CHECK)"
            sig_color = "#8f96a3"
