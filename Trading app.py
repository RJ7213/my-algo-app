...
raw = json.load(open('data_raw.json')) if os.path.exists('data_raw.json') else {'live_spot': 24175.65, 'last_update': '00:00:00'}
p_dt = json.load(open('paper_signal.json')) if os.path.exists('paper_signal.json') else {'rsi_v':44.2,'ema9':24148.59,'rsi_status':'FAIL','ema_status':'FAIL','vol_status':'FAIL','runway_status':'FAIL','vol_val':'1.0x','runway_val':'0 pts','intraday_high':24188.30,'intraday_low':24076.85,'algo_reason':'Processing...','signal_active':False,'active_trade_symbol':'NONE'}
...
if p_dt.get('signal_active'):
    trade_card_html = f"""<div style="background:#00e67615; border:2px solid #00e676; padding:10px; border-radius:12px; color:white; font-family:sans-serif; margin-top:10px; font-size:12px;">🚀 <b>ACTIVE POSITION:</b> {p_dt.get('active_trade_symbol')} | Target Riding Locked!</div>"""
...
<div style="background-color:#060814; padding:15px; border-radius:12px; color:white; border: 1px solid #1c2136; font-family:sans-serif; line-height: 1.4; height: 500px;">
    <div style="background-color:#00e67610; border:1px solid #00e67650; padding:10px; border-radius:10px; text-align:center; margin-bottom:12px;">
        <h1 style="font-size:36px; margin:2px 0; color:#00e676; font-weight:bold;">{raw.get('live_spot'):.2f}</h1>
    </div>
    ...
    <div style="background:#1c2136; border-radius:8px; padding:8px; font-size:11px; margin-top:10px; border-left:4px solid #ffb300;">🧠 <b>ALGO ANALYZER:</b> {p_dt.get('algo_reason')}</div>
...
else: st.caption("⏳ No trades recorded yet. Waiting for market setup...")
