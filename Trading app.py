...
def load_live_signal():
    if not os.path.exists('data_signal.json'): 
        return {'live_spot': 24145.85, 'rsi_v': 44.2, 'ema9': 24148.59, 'nifty_status': '⏳ Waiting for Worker...', 'rsi_status': 'FAIL', 'ema_status': 'FAIL', 'vol_status': 'FAIL', 'runway_status': 'FAIL', 'last_update': '00:00:00'}
    try:
...
dhan_html = f"""
<div style="background-color:#060814; padding:15px; border-radius:12px; color:white; border: 1px solid #1c2136; font-family:-apple-system,BlinkMacSystemFont,sans-serif; line-height: 1.4; height: 380px;">
    ...
    <table style="width:100%; font-size:13px; border-collapse:collapse; background:#111422; border-radius:10px; overflow:hidden; border: 1px solid #1c2136;">
        ...
        <tbody>
            ...
            <tr><td style="padding:8px;">3. Volume Tower</td><td style="padding:8px; text-align:center; color:#fff;">{data.get('vol_val', '1.0x')}</td><td style="padding:8px; text-align:right;">{map_pf(data.get('vol_status'))}</td></tr>
            <tr><td style="padding:8px;">4. Runway Breakthrough</td><td style="padding:8px; text-align:center; color:#fff;">{data.get('runway_val', '0 pts')}</td><td style="padding:8px; text-align:right;">{map_pf(data.get('runway_status'))}</td></tr>
        </tbody>
    </table>
</div>"""

components.html(dhan_html, height=400, scrolling=False)
