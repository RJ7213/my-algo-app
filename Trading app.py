...
# ओआय आणि ऑर्डर बुक डॅशबोर्ड टेस्टिंगसाठी फ्रंट-एंडला नेहमी पास ठेवणे
vol_val_display = p_dt.get('vol_val') if p_dt.get('vol_val') else "1.0x Speed"

trade_card_html = ""
if p_dt.get('signal_active') or p_dt.get('signal_triggered'):
    trade_card_html = f"""<div style="background:#00e67615; border:2px solid #00e676; padding:10px; border-radius:12px; color:white; font-family:sans-serif; margin-top:10px; font-size:12px; text-align:center;">🚀 <b>ACTIVE POSITION:</b> {p_dt.get('active_trade_symbol', 'NIFTY OPTION')} <br><span style="font-size:10px; color:#8f96a3;">15% Risk & Trailing Protection Active</span></div>"""
